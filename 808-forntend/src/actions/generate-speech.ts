...
export async function generateTextToSpeech(
  text: string,
  voice: string,
  service: "styletts2" | "gemini-tts" = "gemini-tts"
) {
...
export async function generateMultiSpeakerTTS(
  ...
  const audioClipJob = await db.generatedAudioClip.create({
    data: {
      text: text,
      // Store speakers as JSON in the voice field
      voice: JSON.stringify(speakers),
      user: {
        connect: {
          id: session.user.id,
        },
      },
      service: "gemini-multi",
    },
  });
...
const revalidateBasedOnService = async (service: ServiceType) => {
  switch (service) {
    ...
    case "seedvc":
      ...
      break;
    case "gemini-multi":
      revalidatePath("/app/speech-synthesis/multi-speaker");
      break;
    case "make-an-audio":

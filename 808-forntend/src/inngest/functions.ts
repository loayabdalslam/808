...
export const aiGenerationFunction = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const result = await step.run("call-api", async () => {
      ...
      } else if (audioClip.service === "gemini-tts") {
        response = await fetch(env.SEED_VC_API_ROUTE + "/tts", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: env.BACKEND_API_KEY,
      ...
      } else if (audioClip.service === "gemini-multi") {
        // Parse the speakers from the voice field
        const speakers = JSON.parse(audioClip.voice || "{}");

        response = await fetch(env.SEED_VC_API_ROUTE + "/multi-speaker", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: env.BACKEND_API_KEY,
